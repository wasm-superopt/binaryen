...
//
// Initial idea was based on Bansal, Sorav; Aiken, Alex (21–25 October 2006): "Automatic Generation of Peephole Superoptimizers".
//
// Current plan:
//  * Generate random function bodies for a function with fixed params and result type (say, two i32s in, i32 out)
//   * If one has side effects, skip it.
//   * Optimize it. We only care about what is not yet possible by the optimizer.
//   * If we encountered it before, skip it.
//   * Note it in our database of seen expressions.
//   * Note its hashed execution results in our database of seen hashes.
//  * Find identical hash results for different expressions, suggesting an optimization may be possible.
//   * Perform more rigorous checks, trying many random inputs, runner a solver to find a counterexample, etc.
//   * This gives us a bunch of possible rules, but we don't know how useful they may be.
//  * Run them on real-world code?
...
// this seems to be off by 1? with 2, we end up with 1 local max
#define MAX_LOCAL 2
...
static Expression* normalize(Expression* expr, Module& wasm) {
  ...
  if (!isConcreteType(ret->type) || normalizer.nextLocal >= MAX_LOCAL || Measurer::measure(ret) > MAX_EXPRESSION_SIZE) {
    return nullptr;
  }
  return ret;
}
...
static bool alreadyOptimizable(Expression* input, Type localTypes[MAX_LOCAL], Expression* output) {
  Module temp;
